package com.winginrian.hibernate.dto;
// Generated 2019-3-20 0:37:34 by Hibernate Tools 5.2.11.Final

/**
 * Processeventrelation generated by hbm2java
 */
public class Processeventrelation implements java.io.Serializable {

	private Integer id;
	private Integer processId;
	private Integer fatherEventId;
	private Integer sonEventId;

	public Processeventrelation() {
	}

	public Processeventrelation(Integer processId, Integer fatherEventId, Integer sonEventId) {
		this.processId = processId;
		this.fatherEventId = fatherEventId;
		this.sonEventId = sonEventId;
	}

	public Integer getId() {
		return this.id;
	}

	public void setId(Integer id) {
		this.id = id;
	}

	public Integer getProcessId() {
		return this.processId;
	}

	public void setProcessId(Integer processId) {
		this.processId = processId;
	}

	public Integer getFatherEventId() {
		return this.fatherEventId;
	}

	public void setFatherEventId(Integer fatherEventId) {
		this.fatherEventId = fatherEventId;
	}

	public Integer getSonEventId() {
		return this.sonEventId;
	}

	public void setSonEventId(Integer sonEventId) {
		this.sonEventId = sonEventId;
	}

}
